...
import type { NotificationMetadata } from "#features/notifications/domain/notification";
 ...
 *
 * Dispatched by the SQS consumer (later task); the command object carries the
 * envelope so it stays usable outside HTTP.
 */
export class CreateNotificationCommand {
  constructor(public readonly envelope: NotificationEnvelope) {}
}
...
export class CreateNotificationHandler implements ICommandHandler<CreateNotificationCommand> {
  constructor(
    ...
    @Inject(PUBLISH_TO_USER) private readonly publishToUser: PublishToUser,
  ) {}

  async execute({ envelope }: CreateNotificationCommand): Promise<"created" | "discarded"> {
    trace
      .getActiveSpan()
      ?.setAttributes({ app_event: "notification_created_started", event_type: envelope.type });
  ...
  private async push(
    ...
    await this.publishToUser(cognitoSub, {
