...
import { SQS_CLIENT } from "#shared/messaging/messaging.module";
import {
...
/**
 * Consumes the notifications queue inside the Users process, sharing the Nest
 * container, Prisma client and logger with the HTTP surface.
 *
 * CONTRACT: Constructed by the module but STARTED from main.ts — never in a
 * constructor or onModuleInit. Test.createTestingModule compiles this module
 * in many tests, and a live long-poll in every run would receive and DELETE
 * real messages outside any test's control — the same reason the metrics
...
export class NotificationConsumerService {
  ...
  constructor(
    ...
    this.consumer = Consumer.create({
      queueUrl: config.get("NOTIFICATIONS_QUEUE_URL"),
      sqs: sqsClient,
  ...
  async handleMessage(message: Message): Promise<void> {
    ...
    return context.with(parentContext, () =>
      ...
      runWithLogContext(
        ...
        async () => {
          await this.commandBus.execute(new CreateNotificationCommand(parsed));
        },
      ),
    );
  }
