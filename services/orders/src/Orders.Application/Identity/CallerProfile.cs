...
/// address it snapshots (<c>null</c> when none is on file), the Stripe customer id
/// charges against (<c>null</c> until the caller has one), and the Cognito sub Users has on
/// file (<c>null</c> when it reports none).
