...
{
    /// <summary>
    /// The liveness probe's route; only its 2xx responses are exempt from the request log.
    /// CONTRACT: A separate constant, NOT <see cref="IsPublic"/> — that predicate also matches
    /// the E2E cleanup route, and exempting a destructive operation from the request log would
    /// hide it. See [[health-check-logging]]
    /// </summary>
