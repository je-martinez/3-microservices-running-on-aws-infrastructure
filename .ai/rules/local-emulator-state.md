---
name: local-emulator-state
description: Floci's local AWS emulator state lives in the floci-state named volume and must be destroyed together with its containers. `make clean` is destructive with no prompt; `make doctor` cross-checks emulator state against docker ps.
paths:
  - "**/docker-compose*.yml"
  - "**/docker-compose*.yaml"
  - "**/Makefile"
