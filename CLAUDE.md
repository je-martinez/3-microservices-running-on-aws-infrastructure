...
- **The test is the TENSE, not a word list.** Read each sentence back before committing it: *does this describe the code as it stands, or tell the reader what changed?* Past tense about THIS CODEBASE is the violation — `became`, `was broken`, `we switched`, `after the fix` are spellings of it, and the linter's marker list is a net, not the rule. Past tense about RUNTIME ("a cart the order just consumed") is fine. This rule was broken twice by the main session in one day, the second time *after* extending the linter, because the offending word (`became`) was not on any list.
- **A comment that restates the code earns nothing.** `?? undefined` already says "or nothing". Write only what the code cannot: the prohibition, the failure it prevents, the non-local reason — and prefer deleting a comment to padding it.
