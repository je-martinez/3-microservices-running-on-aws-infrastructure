...
env-file: ## Refresh Cognito IDs in ./.env from terraform outputs (preserves other vars)
	@# Floci mints a new user-pool/client ID on every apply, so these two must be
	...
	@# The auto-generated Cognito lines are wrapped in a labeled box so it's clear
	...
	mv $$out $(ENV_FILE); \
	rm -f $$rest
	@echo "refreshed the AUTO-GENERATED Cognito box in $(ENV_FILE) (other vars preserved)"
