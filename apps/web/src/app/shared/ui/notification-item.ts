 ...
 * dot, both driven by `notification().readAt` rather than two templates.
 *
 * CONTRACT: `metadata.status` is OPTIONAL — a WELCOME row carries none and falls
 * back to a plain `bell` glyph. `TrackingStatusIcon` requires a non-null
