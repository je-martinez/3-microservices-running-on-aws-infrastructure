...
// Overlay state (cart, account menu, notifications, toast) is UI layered over
// `/` — see DESIGN.md "Overlays are not routes" — and gets no entry here.
export const routes: Routes = [
  {
    // CONTRACT: The app layout stays FIRST of the two `''` parents. A
    // component-less-parent path of '' matches by prefix and consumes the
    // empty URL even when none of its children match, so whichever layout is
    // declared first answers `/`. With the auth layout first, `/` renders the
    ...
    children: [
      {
        ...
        // parent above. On the parent it makes '' the only URL that resolves
        // and drops every sibling to the wildcard; missing here it makes home
        // match its siblings' paths too.
        path: '',
