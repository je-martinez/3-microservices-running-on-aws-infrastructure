...
# only in how they reach Floci — the container by service name, `ng serve` by
...
#
# CONTRACT: The key is READ AT REQUEST TIME, never interpolated into this
# template. The rendered .mjs is gitignored but still sits on disk, and a key
# baked into it would outlive every `make clean`.
# See [[2026-09-06-address-geocoding-proxy-design]]
WEB_PROXY_MODULE = """\
// GENERATED by `make env-file` — do not edit. Floci mints a new <api-id> on
// every apply. Contract: apps/web/proxy.conf.example.mjs
import {{ readFileSync }} from 'node:fs';
import {{ fileURLToPath }} from 'node:url';
import {{ dirname, resolve }} from 'node:path';

// CONTRACT: Read the key from .env.local.web — the same CUSTOM box nginx reads,
// so `ng serve` and the container never disagree about which key is in play.
// An exported GEOAPIFY_API_KEY wins, for a one-off override.
...
export default {{
  ...
  '/geocode/': {{
    ...
    secure: true,
    // WHY: Geoapify sits behind Cloudflare, which needs SNI and Host to name
    // its own vhost — the same requirement nginx.conf documents.
    changeOrigin: true,
    // CONTRACT: Fail CLOSED on an empty key, and answer from HERE. A keyless
    // call answers 401 AND still burns a request off the 3,000/day free tier,
    // so the request must never leave this origin. Ending the response inside
    // `bypass` is what stops Vite from proxying it — returning a dead path
    // instead still reaches Geoapify and gets Cloudflare's HTML 503, which the
    // caller cannot parse. Body matches nginx.conf's byte for byte.
    bypass: (req, res) => {{
...
def write_web_proxy_config(repo_root: Path, api_id: str) -> Path:
    """Write apps/web/proxy.conf.mjs — the `ng serve` half of BOTH proxies.

    Not an env file, so it is written here rather than through write_env_file:
    Angular's dev server takes a proxy module, and it has no CUSTOM box to
    preserve. Gitignored and regenerated, for the same reason the env files are.

    CONTRACT: Emit a MODULE, not JSON. /geocode/ appends a key that must never
    be written to disk, and only a module can read one at request time.
    @angular/build loads any non-.json proxy path as CJS or ESM and honours its
    default export. See [[2026-09-06-address-geocoding-proxy-design]]
    """
    path = repo_root / "apps" / "web" / "proxy.conf.mjs"
    # CONTRACT: `$default` stays a LITERAL here. Vite's rewrite performs no
    # expansion, so it is written plainly — unlike apps/web/nginx.conf, where
    # nginx would parse it as a variable and refuse to start.
    ...
    return {
        ...
        repo_root / ".env.local.orders": dict(
            ...
            custom_defaults={
                ...
                # restricted rk_test_ key (never Users' key), and the whsec_ that
                # `make stripe-webhook-secret` writes. Seeded empty so the keys
                # are visible; Orders reads an empty value as unset (503).
        ...
        repo_root / ".env.local.web": dict(
            ...
            generated={
                ...
                # gateway is unreachable; the host-facing equivalent lives in
                # apps/web/proxy.conf.json for `ng serve`.
                "API_GATEWAY_PROXY_HOST": WEB_PROXY_CONTAINER_HOST,
...
def main(argv: list[str]) -> int:
    ...
    # The `ng serve` half of the same proxy contract. Its api id is read back
    # from the web spec rather than resolved again, so the two proxies cannot
    # drift onto different gateways.
    web_spec = files[args.repo_root / ".env.local.web"]
    proxy_path = write_web_proxy_config(
