...
def write_web_proxy_config(repo_root: Path, api_id: str) -> Path:
    """Write apps/web/proxy.conf.json — the `ng serve` half of the /v1 proxy.

    Not an env file, so it is written here rather than through write_env_file:
    Angular's dev server takes a JSON proxy config, and it has no CUSTOM box to
    preserve. Gitignored and regenerated, for the same reason the env files are.
    """
    path = repo_root / "apps" / "web" / "proxy.conf.json"
    # CONTRACT: `$default` stays a LITERAL here. http-proxy-middleware performs
    # no expansion, so it is written plainly — unlike apps/web/nginx.conf, where
    # nginx would parse it as a variable and refuse to start.
    # See [[floci-vs-ministack-spike-findings]]
    config = {
        "//": (
            "GENERATED by `make env-file` — do not edit. Floci mints a new "
            "<api-id> on every apply. Contract: apps/web/proxy.conf.example.json"
        ),
        "/v1": {
            "target": WEB_PROXY_HOST_TARGET,
            "secure": False,
            "changeOrigin": False,
            "pathRewrite": {
                "^/v1": f"/restapis/{api_id}/$default/_user_request_/v1",
            },
        },
    }
