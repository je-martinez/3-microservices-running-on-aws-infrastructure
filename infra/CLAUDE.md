...
`floci` → `terraform apply` → regenerate `./.env` from outputs → build/start
`users` → `bootstrap.sh` (least-privilege DB user + `nginx-stable` alias). The
order matters: `users` validates `COGNITO_*` with Zod at boot, and those IDs only
exist after apply — and Floci mints new ones on every apply, so `.env` is
