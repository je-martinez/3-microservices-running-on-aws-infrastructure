...
| Block 4 — Tooling & verification | JE-170–JE-172, JE-174, JE-175 | The `pencil-design-extraction` skill + `web-impl` agent, the vault convention note recording the extraction rules, navigation E2E covering every phase-1 route, a refactor extracting inline templates to `.html` and converting `px` to `rem`, and E2E date assertions pinned to a fixed timezone. |
...
| 12 | [JE-174](https://linear.app/je-martinez/issue/JE-174) | Extract templates to `.html` and convert `px` to `rem` | Every component's inline template moved to its own `.html` file; hard-coded `px` values converted to `rem` | [[2026-08-17-web-app-foundation-design]] |
| 13 | [JE-175](https://linear.app/je-martinez/issue/JE-175) | Assert rendered dates and pin a timezone in the web E2E project | Playwright config pins a fixed timezone; specs assert rendered date strings instead of skipping them | [[2026-08-17-web-app-foundation-design]] |
...
3. **Block 3 → Block 4.** The final batch — skill/agent, convention note, navigation E2E, the template/`rem` refactor, and the E2E timezone fix — is reviewed together as the milestone's closing set.
