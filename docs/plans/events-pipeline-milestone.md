...
| Block C — Email | JE-133–JE-135 | `USER_CREATED` handler (react-email + Mailpit + preview server + SES sender identity — first end-to-end email), `ORDER_CREATED` (proves the "one dispatch-map entry" claim), and the `TRACKING_STATUS_CHANGED` template family (one event type, four rendered variants by `payload.status`). |
...
| 12 | [JE-135](https://linear.app/je-martinez/issue/JE-135) | `TRACKING_STATUS_CHANGED` template family + handler | One event type, four catalog entries sharing one `TrackingStatusChangedEmail` component, selected by `payload.status` | [[2026-08-03-events-pipeline-milestone-design]] |
